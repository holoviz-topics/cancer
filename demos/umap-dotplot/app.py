...
class CellViewer(pn.viewable.Viewer):
    ...
    result = param.Selector(default="cell_type")
    ...
    def __init__(self, adata, obs_df, marker_genes, expression_cutoff=0.1, **params):
        ...
        self.param["result"].objects = sorted(
            [
                key
                for key, series in adata.obs.items()
        ...
        self.template = pn.template.FastListTemplate(
            ...
            sidebar=[self.param.result, self.param.max_dot_size],
    ...
    @pn.depends("result", "max_dot_size", watch=True)
    def _load(self):
    ...
    def _plot_umap_points(self):
        umap_points = self.obs_df.hvplot.points(
            ...
            cmap="Category20",
            datashade=True,
            aggregator=ds.count_cat(self.result),
            tools=["box_select"],
            legend=False,
            grid=False,
            height=500,
        ...
        return linked_umap_points * labels * inspect_selection # * labels_glow
    ...
    def _prepare_dendrogram_data(self, dp_df, cluster_positions):
        ...
        clusters_ordered = self._order_cluster_labels(cluster_gene_matrix.index)
    ...
    def _plot_dp_points(self, dp_df):
        ...
        cluster_positions = {label: idx for idx, label in enumerate(self._order_cluster_labels(cluster_labels))}
